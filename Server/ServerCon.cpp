...
static int iCount;
ServerCon::ServerCon(CServerDlg* dialog)
{
	m_pDialog = dialog;
}
...
void ServerCon::ClearServer()
{
    closesocket(s);
    WSACleanup();
}

void ServerCon::StartListening(int iPort)
{
    iCount = 0;
...
UINT __cdecl ServerCon::DataThreadFunc(LPVOID pParam)
{
...
UINT ServerCon::SendReceiveData(SOCKET cSocket)
{
    return 0;
}
void ServerCon::SetStaticVariable(int ic, SOCKET cS)
{
    iCount = ic;
    sArray[iCount] = cS;
}
